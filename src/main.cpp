...
int nCoinbaseMaturity = 45; //120 blocks for coins to mature gnome sayan
...
const string strMessageMagic = "orthocoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    } else if(nHeight >= 2 && nHeight <= 5760) {
    nSubsidy = 0 * COIN;
    } else if(nHeight >= 5761 && nHeight <= 5961) {
    nSubsidy = 1 * COIN;
    } else if(nHeight >= 5962 && nHeight <= 7200) {
    nSubsidy = 50 * COIN;
    } else if(nHeight >= 7201 && nHeight <= 10080) {
    nSubsidy = 100 * COIN;
    } else if(nHeight >= 10081 && nHeight <= 14400) {
    nSubsidy = 75 * COIN;
    }
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "orthocoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "OrthoCoin - Our projection is to the top";
        CTransaction txNew;
        txNew.nTime = 1411068391;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x479c57c22422015281217179d07ef09ea8ace7211a323ffea2f8a600f00212c3"));
        //block.print();
